...
ShadowMapPSO::ShadowMapPSO(std::shared_ptr<Device> _device, bool _isDecal)
	:BasePSO(_device),
	m_IsDecal(_isDecal)
{
	...
	if (m_IsDecal)
	{
		ThrowIfFailed(D3DReadFileToBlob(L"C:\\Code\\DX12New\\x64\\Debug\\ShadowMap_Decal_PS.cso", &pixelShaderBlob));
	}
	else
	{
		ThrowIfFailed(D3DReadFileToBlob(L"C:\\Code\\DX12New\\x64\\Debug\\ShadowMap_PS.cso", &pixelShaderBlob));
	}
...
void ShadowMapPSO::Apply(CommandList& _commandList)
{
